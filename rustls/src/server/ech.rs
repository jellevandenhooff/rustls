...
pub trait EchKeyResolver: Debug + Send + Sync {
    /// Return the current ECH key index.
    fn resolve(&self) -> Arc<EchKeyIndex>;
}
...
/// immutable key index.
#[derive(Debug)]
pub struct FixedEchKeys(Arc<EchKeyIndex>);
...
impl EchKeyIndex {
                ...
                .symmetric_cipher_suites
            {
                ...
                for &hpke_suite in &key.hpke_suites {
                    if hpke_suite.suite() == expected {
                        by_config_id[key.config_id() as usize].push(EchKeyEntry {
                            cipher_suite,
                            key_index: i,
                            hpke_suite,
                        });
    ...
    /// Iterate over candidate (key, hpke_suite) pairs matching the given
    /// config_id and cipher_suite.
    fn candidates(
        &self,
        config_id: u8,
        cipher_suite: HpkeSymmetricCipherSuite,
    ) -> impl Iterator<Item = (&EchServerKey, &'static dyn Hpke)> {
        self.by_config_id
            ...
            .map(move |e| (&self.keys[e.key_index], e.hpke_suite))
    }
...
impl EchServerState {
    /// Initial state before ECH resolution.
    ...
    /// The config_id from the accepted ECH frontend, if any.
    ...
    /// Resolve ECH for the initial ClientHello.
    ///
    /// Returns the decrypted inner ClientHello if ECH was accepted,
    /// or `None` to proceed with the outer ClientHello.
    ///
    /// `accept_inner_direct` controls whether a ClientHello with the
    /// `ech_is_inner` marker (split-mode backend) is accepted.
    pub(crate) fn resolve<'m>(
        &mut self,
        index: &EchKeyIndex,
        input: &'m Input<'m>,
        accept_inner_direct: bool,
        ...
        match &outer_hello.encrypted_client_hello {
            Some(EncryptedClientHello::Inner) if accept_inner_direct => {
                self.backend = Some(EchBackend {
                    inner_random: outer_hello.random,
                });
                Ok(None)
            }
            Some(EncryptedClientHello::Outer(_)) if !index.is_empty() => {
                let outer = OuterClientHello::from_input(input)?;
                match EchFrontend::decrypt(&outer, index)? {
    ...
    ///
    /// Uses the HPKE context from the initial resolve to decrypt the
    /// second ClientHello. Returns the decrypted inner ClientHello if
    /// ECH was accepted, or `None` to proceed with the outer.
    pub(crate) fn resolve_retry<'m>(
        &mut self,
        input: &'m Input<'m>,
    ) -> Result<Option<Input<'m>>, Error> {
        let Some(backend) = &mut self.backend else {
            // No backend means ECH was not accepted -- nothing to do on HRR.
            return Ok(None);
        };
        ...
        if let Some(frontend) = &mut self.frontend {
            ...
            // Inner-direct: update inner_random if the retry hello has the marker.
            let outer_hello = require_handshake_msg!(
                input.message,
                HandshakeType::ClientHello,
                HandshakePayload::ClientHello
            ...
            if matches!(
                ...
                backend.inner_random = outer_hello.random;
            }
            Ok(None)
        }
    }
...
impl EchFrontend {
    ...
    fn decrypt(
        ...
        match outer.decrypt_ech(index) {
            ...
            EchDecryptResult::InnerDirect => Ok(None),
...
///
/// Only exists when ECH was accepted (via decryption or inner-direct).
...
impl<'a> OuterClientHello<'a> {
    ...
    fn decrypt_ech(&self, index: &EchKeyIndex) -> EchDecryptResult {
        ...
        for (key, hpke_suite) in candidates {
            let Ok(mut opener) = hpke_suite.setup_opener(&enc, &key.hpke_info, &key.private_key)
            else {
                continue;
            };
