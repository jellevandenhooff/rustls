...
pub struct ServerConfig {
    ...
    /// Whether to accept ClientHellos with the `ech_is_inner` marker
    /// (split-mode backend).
    ///
    /// When `true`, the server accepts inner ClientHellos forwarded by a
    /// split-mode frontend proxy. When `false` (the default), the marker
    /// is ignored and the ClientHello is treated normally.
    ///
    /// Only enable this on servers that sit behind a trusted ECH frontend.
    pub accept_ech_inner_direct: bool,
